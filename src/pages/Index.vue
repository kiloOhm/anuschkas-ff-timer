...
import { useRealtime } from '../util/realtime';
  ...
  const { sessionId, connectedToPubSub, setMode } = useRealtime();
...
<template>
  ...
  <n-button @click="generateAudioFile" :loading="generatingAudio" ghost circle class="absolute! bottom-4 left-4 hover:opacity-100 transition-opacity before:absolute before:-inset-4 before:block" :class="{'opacity-0': !generatingAudio}">
    ...
    <n-button v-if="connectedToPubSub && sessionId" @click="copySessionId" ghost class="absolute! bottom-4 right-4 opacity-0 hover:opacity-100 transition-opacity before:absolute before:-inset-4 before:block">
      <span>{{ sessionId }}</span>
    </n-button>
  </n-config-provider>
</template>
